...
<Warning title="为什么不使用 Vuex 而使用 Pinia？">
...
<Note title="提示：如果你的应用使用的 Vue 版本低于 2.7，你还需要安装组合式 API 包：@vue/composition-api">
</Note>
...
<Warning title="如果你学过 Java，可以将其看做是一个公开的静态类，所有的属性和方法都是静态的。">
</Warning>
...
<Warning title="注意：pinia 提供的 storeToRefs 只会将数据做转换，而 Vue 的 toRefs 会转换 store 中所有的数据。推荐使用 storeToRefs。">
</Warning>
...
<Note title="函数式修改时，会传一个 state 的参数，这个参数是 store 的内部状态，可以随意命名，只要与使用时保持一致即可。">
</Note>
...
<Warning title="Pinia 章节总结">
到此，Pinia 章节就算结束了，本章节的代码演示都以组合式 API 风格为主，也推荐使用这种方法。
</Warning>
