...
```
用户提问 → 检索相关文档 → 将文档 + 问题一起送入 LLM → 生成有依据的回答
```
...
│                     离线索引阶段                          │
│                                                         │
│  原始文档 → 文档分块 → 文本嵌入(Embedding) → 向量数据库   │
└─────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────┐
│                     在线查询阶段                          │
│                                                         │
│  用户提问 → 查询嵌入 → 向量检索 → 获取相关文档片段         │
│      ↓                                                  │
│  构建 Prompt（问题 + 检索结果） → LLM 生成回答            │
└─────────────────────────────────────────────────────────┘
```
...
员工提问："公司的年假制度是怎样的？"
   ↓
检索到：《员工手册》第三章 - 休假制度相关内容
   ↓
LLM 生成：根据公司规定，正式员工入职满一年后享有 5 天带薪年假...
```
