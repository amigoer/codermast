...
```mermaid
flowchart LR
    Roots["GC Roots"] --> O1["Object 1"]
    Roots --> O4["Object 4"]
    O1 --> O2["Object 2"]
    O1 --> O3["Object 3"]
    O5["Object 5"] --> O6["Object 6"]
    O5:::unreachable
    O6:::unreachable
    classDef unreachable stroke-dasharray: 5 5
```

> Object 5 与 Object 6 不可达，可回收。
...
```mermaid
flowchart TD
    subgraph Young["年轻代 #40;复制算法，对象存活率低#41;"]
        Eden["Eden"]
        S0["S0"]
        S1["S1"]
    end
    subgraph Old["老年代 #40;标记-清除 / 标记-整理，对象存活率高#41;"]
...
```mermaid
flowchart LR
    U1["用户线程 运行"] --> Pause["暂停 STW"]
    Pause --> U2["用户线程 运行"]
    Pause --> GC["GC 线程 #40;Serial 单线程#41;"]
```
...
```mermaid
flowchart LR
    U1["用户线程 运行"] --> Init["初始标记 STW"]
    Init --> Concurrent["并发标记 #40;与用户线程并发#41;"]
    Concurrent --> Remark["重新标记 STW"]
    Remark --> Sweep["并发清除 #40;与用户线程并发#41;"]
    Sweep --> U2["用户线程 运行"]
```
